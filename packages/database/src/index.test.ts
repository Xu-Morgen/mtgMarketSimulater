...
describe("database foundation", () => {
  it("migrates an empty database and enables SQLite safety pragmas", () => {
    ...
    expect(database.prepare("SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'booster_packs'").get()).toEqual({ name: "booster_packs" });
    expect(database.prepare("SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'pack_rule_replays'").get()).toEqual({ name: "pack_rule_replays" });
