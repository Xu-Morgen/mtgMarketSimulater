...
test("管理员可确认触发同步；等待任务时禁止重复提交且不会请求外部 Provider", async ({ page }) => {
  ...
  await page.route("**/v1/admin/catalog/sync", async (route) => {
    ...
    return route.fulfill({ contentType: "application/json", body: JSON.stringify(envelope({ latestSuccessful: successRun, current: successRun, currentJob })) });
  });
...
test("管理员可查看失败摘要和旧目录，并二次确认重试失败任务", async ({ page }) => {
  await loginAdmin(page);
  let retryCalls = 0;
  await page.route("**/v1/admin/catalog/sync", async (route) => route.fulfill({ contentType: "application/json", body: JSON.stringify(envelope({ latestSuccessful: successRun, current: failedRun, currentJob: { id: jobId, type: "catalog.sync", status: "failed", attempt: 1, maxAttempts: 3, uniqueKey: "catalog.sync:fixture", scheduledAt: now, lockedUntil: null, lastError: "Scryfall Bulk 文件下载失败：HTTP 503", updatedAt: now } })) }));
