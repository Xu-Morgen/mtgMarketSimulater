...
test("玩家闭环从工作资金经获得卡牌、交易、构筑、比赛、成就到再投资均有连续浏览器入口", async ({ page }) => {
  ...
  await page.route("**/v1/onboarding", (route) => route.fulfill({ contentType: "application/json", body: JSON.stringify(envelope({ onboarding: { ruleVersion: "onboarding/v3", steps: [], completedCount: 0, totalCount: 0, allCompleted: true, currentStepId: null, reward: { status: "claimed", amount: money(500), claimedAt: now }, updatedAt: now } })) }));
