...
全量套件在本地最多使用 4 个 worker，在 CI 使用 1 个 worker，避免桌面/窄屏 Chromium 与 Next dev 按需编译争抢资源导致伪超时。排障时可传 `--workers=1` 串行运行；命令行参数会覆盖配置默认值。
...
`decks.spec.ts` 是 I24F Commander 卡组页面回归：只模拟本地卡组、库存和合法性端点，覆盖空库存、无限虚拟基本地、卡组总张数、名称必填提示、Companion、服务端合法/非法结果、报名评分未生成、订单/比赛锁定冲突与未保存离开提示；并覆盖库存表格的费用/颜色、类别（含传奇）、攻防、复用库存页的悬浮图片预览、传奇生物指挥官入口、选定指挥官后的自动颜色筛选、主牌/指挥官候选隐藏，以及固定十行分页。浏览器不实现 Commander 规则、评分或锁定。

`tournaments.spec.ts` 是 I25F 比赛页面回归：以服务端夹具覆盖今日无赛事、报名确认/重复点击/服务端非法卡组、立即或截止结算状态、随机补充包奖励、NPC 公开重放、历史结果，以及现实桌轮次的同桌结果提交和越权拒绝。测试不在浏览器计算积分、排名、奖励或权限。

`achievements.spec.ts` 是 I26F 成就与收藏里程碑回归：只模拟成就只读查询，覆盖空态、重复刷新不产生浏览器写请求、已解锁自动发放/风控拦截状态，以及赛事、收藏和账本来源跳转。测试不模拟或调用解锁、发奖、补发命令。
