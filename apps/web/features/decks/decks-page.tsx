...
function InventoryPicker({ holdings }: { holdings: InventoryHoldingDto[] }) {
  ...
  const hasCommanderCandidate = holdings.some((holding) => isLegendaryCreature(holding.sku.typeLine));
  const hasSelectedCommander = cards.some((card) => card.zone === "commander");
  const firstCommanderSkuId = hasSelectedCommander ? undefined : shown.find((holding) => isLegendaryCreature(holding.sku.typeLine))?.skuId;
