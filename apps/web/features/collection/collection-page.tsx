...
export function CollectionPage() {
  ...
  return <main className="page collection-page"><p className="eyebrow">服务端收藏快照</p><h1>收藏册</h1><p className="intro">进度、市值和未报价提示均由服务端聚合。选择卡牌可查看本地目录详情；库存和比赛跳转不会修改任何持仓或报名。</p>
    <section className="balance-grid notice-board" aria-label="收藏册进度">
      <article><span>已收集 SKU</span><strong className="num">{overview.collection.distinctSkuCount} 种</strong><small>当前持有 {overview.collection.totalCardCount} 张</small></article>
      <article><span>收藏市值</span><strong className="num">{overview.collection.marketValue ? formatMoney(overview.collection.marketValue) : "暂不可用"}</strong><small>{overview.collection.unpricedSkuCount > 0 ? `${overview.collection.unpricedSkuCount} 种持仓没有有效报价` : "全部持仓已有服务端报价"}</small></article>
      <article><span>赛事与库存</span><strong className="num">{overview.todayTournaments.registeredCount} 场今日已报名</strong><small>比赛锁定量请在库存页查看</small></article>
    </section>
