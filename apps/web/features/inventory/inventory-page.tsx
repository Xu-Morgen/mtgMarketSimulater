...
import type { BilateralOrderDto, CardFinish, BatchNpcSellResultDto, DuplicatesSellResultDto, InventoryHoldingDto, NpcTradeDto } from "@mtg-market/contracts";
...
import { BatchSellDialog, BatchSellResultBanner } from "./batch-sell-dialog";
...
export function InventoryPage() {
  ...
  // I34F（I34B D4）：按当前筛选批量卖出。SKU 列表来自当前页命中（有可用量）的持仓，只提交意图。
  const [batchOpen, setBatchOpen] = useState(false);
  const [batchResult, setBatchResult] = useState<BatchNpcSellResultDto | null>(null);
  ...
  const columns = useMemo<ColumnsType<InventoryHoldingDto>>(() => [
    { title: "SKU / 印刷", key: "sku", render: (_, holding) => (
      <div>
        <div className={styles.skuName}>
          <Link className={styles.nameLink} href={skuHistoryHref(holding.skuId)}>{holding.sku.name}</Link>
    ...
    { title: "服务端现价 / 市值", key: "marketValue", render: (_, holding) => holding.marketValue && holding.marketUnitPrice ? <div><Link className={styles.nameLink} href={skuHistoryHref(holding.skuId)}><strong>{formatMoney(holding.marketUnitPrice)} / 张</strong></Link><br /><span className={styles.secondary}>市值 {formatMoney(holding.marketValue)}</span></div> : <Tag color="default">{unavailableReason(holding.marketValueUnavailableReason)}</Tag> },
  ...
  return <main className="page inventory-page"><p className="eyebrow">服务端库存快照</p><h1>我的库存</h1><p className="intro">数量、成本、现价、市值、盈亏与锁定状态均来自服务端。此页面不提供修改库存或解锁资产的入口。</p>
    ...
    {batchResult ? <BatchSellResultBanner result={batchResult} /> : null}
    <form className="catalog-filters" onSubmit={(event) => { event.preventDefault(); apply(); }}><FilterBar>
      ...
      <button className="button" type="submit">应用筛选</button><button className="button secondary" type="button" onClick={() => { setDraft({ query: "", setCode: "", finish: "", locked: "any" }); router.push("/inventory"); }}>清除</button><Button onClick={() => void inventory.refetch()}>刷新</Button><button className="button secondary" type="button" onClick={() => { setDuplicatesResult(null); setDuplicatesOpen(true); }}>批量卖出重复卡</button><button className="button secondary" type="button" disabled={page.items.length === 0} onClick={() => { setBatchResult(null); setBatchOpen(true); }}>批量卖出当前筛选</button>
    </FilterBar></form>
    ...
    <BatchSellDialog open={batchOpen} skuIds={batchSkuIds} onClose={() => setBatchOpen(false)} onSettled={(result) => { setBatchOpen(false); setBatchResult(result); }} />
  </main>;
}
