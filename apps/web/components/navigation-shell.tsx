...
function Shell({ children, admin }: Readonly<{ children: React.ReactNode; admin: boolean }>) { const { user } = useSession(); const links: Array<{ href: string; label: string }> = admin ? [{ href: "/admin", label: "后台首页" }, { href: "/admin/catalog-sync", label: "目录同步" }, { href: "/admin/price-sync", label: "价格同步" }, { href: "/admin/content", label: "内容" }, { href: "/admin/events", label: "活动" }, { href: "/admin/users", label: "玩家" }, { href: "/admin/jobs", label: "任务" }, { href: "/admin/logs", label: "日志" }] : [{ href: "/dashboard", label: "玩家首页" }, { href: "/catalog", label: "卡牌目录" }, { href: "/packs", label: "补充包商店" }, { href: "/inventory", label: "我的库存" }]; return <div className="app-shell"><header className="topbar"><Link href={admin ? "/admin" : "/dashboard"} className="brand">MTG 市场模拟器</Link><span className="user-label">{user?.displayName} · {admin ? "管理员" : "玩家"}</span><SignOutButton /></header><div className="shell-body"><nav className="side-nav" aria-label={admin ? "管理导航" : "玩家导航"}>{links.map(({ href, label }) => <Link href={href} key={href}>{label}</Link>)}</nav><main className="content">{children}</main></div></div>; }
