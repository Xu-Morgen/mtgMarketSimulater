...
- `/onboarding` 按服务端 `OnboardingDto` 渲染九步完整目标链：创建存档 → 领取工作资金 → 开包 → 看懂价格 → 首笔 NPC 交易 → 实际浏览收藏图鉴 → 保存合法 Commander 卡组 → 首次报名 → 等待并查看已结算赛果。组卡步骤在编辑器依次高亮库存/指挥官、名称输入、虚拟基本地、100 张后的服务端检查和合法后的保存按钮；名称以非空且失焦作为 Tour 交互阶段完成条件。虚拟基本地只开放服务端库存 DTO 中主将 `colorIdentity` 对应的颜色，主将变化时清理草稿中的异色基本地，但该前端约束不替代 `commander-100/v1` 服务端合法性规则。缺少传奇生物时明确引导回市场购入。完成/跳过/待完成/下一步状态与 x/9 进度只取服务端投影。
