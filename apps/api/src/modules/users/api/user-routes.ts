...
export async function registerUserRoutes(app: FastifyInstance, database: Database.Database, config: Pick<ApiConfig, "APP_TIMEZONE" | "DAILY_WORK_FUNDING_RULE_VERSION">): Promise<void> {
  ...
  app.get("/v1/daily-work-funding", { preHandler: requireRole("player") }, async (request) =>
    success(request.requestId, { status: users.dailyWorkFundingStatus(request.actor!.id) })
  );
  app.post("/v1/daily-work-funding/claim", { preHandler: requireRole("player") }, async (request, reply) => {
    const key = request.headers["idempotency-key"];
    if (typeof key !== "string" || !isValidIdempotencyKey(key)) return reply.code(400).send(failure(request.requestId, "IDEMPOTENCY_KEY_REQUIRED", "写请求必须携带格式正确的 Idempotency-Key"));
    const body = archiveBodySchema.parse(request.body ?? {});
    const result = users.claimDailyWorkFunding({ userId: request.actor!.id, idempotencyKey: key, requestFingerprint: archiveRequestFingerprint(body), requestId: request.requestId });
    if (result.state === "conflict") return reply.code(409).send(failure(request.requestId, "IDEMPOTENCY_CONFLICT", "Idempotency-Key 已用于不同请求"));
    if (result.state === "in-progress") return reply.code(409).send(failure(request.requestId, "IDEMPOTENCY_IN_PROGRESS", "相同请求正在处理中"));
    if (result.state === "not-open") return reply.code(409).send(failure(request.requestId, "RESOURCE_CONFLICT", "今日工作资金尚未由服务器日切开放"));
    if (result.state === "archive-required") return reply.code(409).send(failure(request.requestId, "RESOURCE_CONFLICT", "尚未创建游戏存档"));
    if (result.state === "already-claimed") return reply.code(409).send(failure(request.requestId, "RESOURCE_CONFLICT", "今日工作资金已领取"));
    if (result.state === "replayed") return reply.code(200).send(result.response);
    return reply.code(201).send(result.response);
  });
}
