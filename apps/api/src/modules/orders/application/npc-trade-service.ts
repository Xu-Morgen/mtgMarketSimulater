...
export class NpcTradeService {
  ...
  buyPreview(userId: string, skuId: string, quantity: number, now = new Date()): NpcBuyPreviewResult {
    const quote = this.market.npcSettlementQuote(skuId);
  ...
  buy(input: {
    ...
    return this.inventory.withLedgerTransaction(() => {
      ...
      const quote = this.market.npcSettlementQuote(input.skuId, input.quoteId);
      ...
      const trade = this.tradeDto(tradeId, input.userId, input.skuId, input.quantity, quote, preview.total.amount, now);
      ...
      ).run(eventId, tradeId, JSON.stringify(event), now);
      this.database.prepare(
  ...
  private tradeDto(id: string, userId: string, skuId: string, quantity: number, quote: NpcSettlementQuote, totalAmount: number, settledAt: string): NpcTradeDto {
    return { id, userId, skuId, side: "buy", quantity, quoteId: quote.quoteId, quoteVersion: quote.quoteVersion, unitPrice: money(quote.unitPriceAmount), unitFee: money(quote.unitFeeAmount), total: money(totalAmount), fee: money(multiply(quote.unitFeeAmount, quantity, "NPC 买入费用")), settledAt };
  }
