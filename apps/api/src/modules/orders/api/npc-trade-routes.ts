...
import { NpcTradeService, npcBuyRequestFingerprint, npcSellBatchRequestFingerprint, npcSellRequestFingerprint } from "../application/npc-trade-service.js";
...
/** I34B（D4）按筛选结果批量卖出：SKU 列表由服务端逐项校验，去重后按既有卖出规则结算。 */
const sellBatchBody = z.object({ skuIds: z.array(z.string().uuid()).min(1).max(100) }).strict();
...
export async function registerNpcTradeRoutes(app: FastifyInstance, database: Database.Database, config: Pick<ApiConfig, "APP_TIMEZONE">): Promise<void> {
  ...
  // I34B（D4）：按筛选结果批量向 NPC 卖出；只提交 SKU 意图，逐 SKU 结算、额度与汇总均由服务端单事务完成。
  app.post("/v1/npc-trades/sell/batch", { preHandler: requireRole("player") }, async (request, reply) => {
    const key = request.headers["idempotency-key"];
    if (typeof key !== "string" || !isValidIdempotencyKey(key)) return reply.code(400).send(failure(request.requestId, "IDEMPOTENCY_KEY_REQUIRED", "写请求必须携带格式正确的 Idempotency-Key"));
    const body = sellBatchBody.parse(request.body);
    const result = trades.sellBatch({ userId: request.actor!.id, skuIds: body.skuIds, idempotencyKey: key, requestFingerprint: npcSellBatchRequestFingerprint(body), requestId: request.requestId });
    if (result.state === "conflict") return reply.code(409).send(failure(request.requestId, "IDEMPOTENCY_CONFLICT", "Idempotency-Key 已用于不同请求"));
    if (result.state === "in-progress") return reply.code(409).send(failure(request.requestId, "IDEMPOTENCY_IN_PROGRESS", "相同请求正在处理中"));
    return reply.code(result.state === "replayed" && result.response.ok ? 200 : result.statusCode).send(result.response);
  });
