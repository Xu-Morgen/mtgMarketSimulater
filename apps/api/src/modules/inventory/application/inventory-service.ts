...
import {
  ...
  type InventoryLockTarget
} from "../domain/inventory.js";
import {
  SqliteInventoryRepository,
  type InventoryFilters
} from "../infrastructure/sqlite-inventory-repository.js";
...
export class InventoryService {
  ...
  constructor(private readonly database: Database.Database) {
    this.inventory = new SqliteInventoryRepository(database);
  }

  withLedgerTransaction<T>(operation: InventoryLedgerTransaction<T>): T {
    return withinTransaction(this.database, () => operation(this.inventory));
  }
  /** 供同一经济事务内的跨模块用例调用；调用方必须已经处于 withLedgerTransaction 回调中。 */
  acquireInLedgerTransaction(input: InventoryAdjustment): InventoryHoldingDto | "insufficient" {
  ...
  acquire(
    ...
    withLedger?: () => void
  ): InventoryHoldingDto | "insufficient" {
    assertPositiveQuantity(input.quantityDelta);
    return this.withLedgerTransaction((inventory) => {
      const result = inventory.adjust(input);
      if (result !== "insufficient") withLedger?.();
      return result;
    });
  }
  lock(input: {
  ...
  holding(userId: string, skuId: string) {
    return this.inventory.findHolding(userId, skuId);
  }
  reconciliation(userId: string, skuId: string, cursor: string | undefined, limit: number) {
    return this.inventory.reconcile(userId, skuId, cursor, limit);
  }
}
