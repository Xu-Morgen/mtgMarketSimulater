...
type HoldingWithCatalogRow = Omit<HoldingRow, "market_value_amount"> & { computed_market_value_amount: number | null; market_unit_price_amount: number | null; unrealized_profit_loss_amount: number | null; name: string; set_code: string; set_name: string; collector_number: string; finish: "nonfoil" | "foil" | "etched"; image_path: string | null; tradable: number; mana_cost: string | null; colors_json: string; color_identity_json: string; type_line: string; power: string | null; toughness: string | null; oracle_text: string | null };
...
export type InventoryFilters = { query?: string | undefined; setCode?: string | undefined; finish?: "nonfoil" | "foil" | "etched" | undefined; locked?: "any" | "locked" | "available" | undefined; sort: "updatedAt" | "name" | "quantity" | "availableQuantity" | "marketValue"; direction: "asc" | "desc"; cursor?: string | undefined; limit: number };

function holdingDto(row: HoldingWithCatalogRow): InventoryHoldingDto {
  return { skuId: row.sku_id, quantity: row.quantity, availableQuantity: row.available_quantity, orderLockedQuantity: row.order_locked_quantity, tournamentLockedQuantity: row.tournament_locked_quantity, averageCost: { amount: row.average_cost_amount, currency: "GAME_CREDIT" }, marketUnitPrice: row.market_unit_price_amount === null ? null : { amount: row.market_unit_price_amount, currency: "GAME_CREDIT" }, marketValue: row.computed_market_value_amount === null ? null : { amount: row.computed_market_value_amount, currency: "GAME_CREDIT" }, unrealizedProfitLoss: row.unrealized_profit_loss_amount === null ? null : { amount: row.unrealized_profit_loss_amount, currency: "GAME_CREDIT" }, updatedAt: row.updated_at, marketValueUnavailableReason: row.computed_market_value_amount === null ? "no_snapshot" : null, sku: { id: row.sku_id, name: row.name, setCode: row.set_code, setName: row.set_name, collectorNumber: row.collector_number, finish: row.finish, imagePath: publicImagePath(row.image_path), tradable: row.tradable === 1, manaCost: row.mana_cost, colors: JSON.parse(row.colors_json) as InventoryHoldingDto["sku"]["colors"], colorIdentity: JSON.parse(row.color_identity_json) as InventoryHoldingDto["sku"]["colorIdentity"], typeLine: row.type_line, power: row.power, toughness: row.toughness, oracleText: row.oracle_text } };
}
...
export class SqliteInventoryRepository {
  ...
  list(userId: string, filters: InventoryFilters): Page<InventoryHoldingDto> {
    ...
    const clause = `WHERE ${where.join(" AND ")}`; const direction = filters.direction.toUpperCase();
    // 排序键均来自白名单枚举，direction 同样经 zod 校验；marketValue 以 `IS NULL` 使无快照持仓在两种方向下都垫后。
    const order = { updatedAt: `h.updated_at ${direction}`, name: `p.name COLLATE NOCASE ${direction}`, quantity: `h.quantity ${direction}`, availableQuantity: `h.available_quantity ${direction}`, marketValue: `computed_market_value_amount IS NULL ASC, computed_market_value_amount ${direction}` }[filters.sort];
    ...
    const rows = this.database.prepare(`${this.selectHoldingSql()} ${clause} ORDER BY ${order}, h.id ASC LIMIT ? OFFSET ?`).all(...values, filters.limit + 1, offset) as HoldingWithCatalogRow[];
  ...
  /** 估值、成本与盈亏均在 SQLite 投影中以整数计算，避免把经济运算下放到浏览器。投影别名特意与物理列区分，以便按计算市值排序时不被物理快照列名遮蔽。 */
  private selectHoldingSql(): string { return "SELECT h.id, h.user_id, h.sku_id, h.quantity, h.available_quantity, h.order_locked_quantity, h.tournament_locked_quantity, h.average_cost_amount, quote.market_price_amount AS market_unit_price_amount, CASE WHEN quote.market_price_amount IS NOT NULL THEN quote.market_price_amount * h.quantity ELSE h.market_value_amount END AS computed_market_value_amount, COALESCE(quote.calculated_at, h.market_value_captured_at) AS market_value_captured_at, CASE WHEN quote.market_price_amount IS NOT NULL THEN quote.market_price_amount * h.quantity - h.average_cost_amount * h.quantity WHEN h.market_value_amount IS NOT NULL THEN h.market_value_amount - h.average_cost_amount * h.quantity ELSE NULL END AS unrealized_profit_loss_amount, h.updated_at, p.name, s.code AS set_code, s.name AS set_name, p.collector_number, sku.finish, image.cache_path AS image_path, sku.tradable, p.mana_cost, p.colors_json, p.color_identity_json, p.type_line, p.power, p.toughness, p.oracle_text FROM inventory_holdings h JOIN card_skus sku ON sku.id = h.sku_id JOIN card_printings p ON p.id = sku.printing_id JOIN card_sets s ON s.id = p.set_id LEFT JOIN card_image_cache image ON image.printing_id = p.id LEFT JOIN market_quotes quote ON quote.rowid = (SELECT latest.rowid FROM market_quotes latest WHERE latest.sku_id = h.sku_id ORDER BY latest.calculated_at DESC, latest.rowid DESC LIMIT 1)"; }
}
