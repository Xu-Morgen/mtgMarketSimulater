...
type HoldingWithCatalogRow = HoldingRow & { market_unit_price_amount: number | null; unrealized_profit_loss_amount: number | null; name: string; set_code: string; set_name: string; collector_number: string; finish: "nonfoil" | "foil" | "etched"; image_path: string | null; tradable: number };
...
function holdingDto(row: HoldingWithCatalogRow): InventoryHoldingDto {
  return { skuId: row.sku_id, quantity: row.quantity, availableQuantity: row.available_quantity, orderLockedQuantity: row.order_locked_quantity, tournamentLockedQuantity: row.tournament_locked_quantity, averageCost: { amount: row.average_cost_amount, currency: "GAME_CREDIT" }, marketUnitPrice: row.market_unit_price_amount === null ? null : { amount: row.market_unit_price_amount, currency: "GAME_CREDIT" }, marketValue: row.market_value_amount === null ? null : { amount: row.market_value_amount, currency: "GAME_CREDIT" }, unrealizedProfitLoss: row.unrealized_profit_loss_amount === null ? null : { amount: row.unrealized_profit_loss_amount, currency: "GAME_CREDIT" }, updatedAt: row.updated_at, marketValueUnavailableReason: row.market_value_amount === null ? "no_snapshot" : null, sku: { id: row.sku_id, name: row.name, setCode: row.set_code, setName: row.set_name, collectorNumber: row.collector_number, finish: row.finish, imagePath: row.image_path, tradable: row.tradable === 1 } };
}
