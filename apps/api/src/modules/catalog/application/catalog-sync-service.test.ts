...
describe("I09B Scryfall Bulk 同步", () => {
  it("记录版本和 checksum，并按启用系列原子导入独立 SKU", async () => {
    ...
    await sync.synchronize({ expectedChecksumSha256: checksum, cacheImageScryfallIds: [card.id] });
    ...
    expect(database.prepare("SELECT source_version, checksum_sha256, status, cached_images FROM catalog_sync_runs").get()).toEqual(expect.objectContaining({ source_version: "2026-07-24T00:00:00.000Z", checksum_sha256: checksum, status: "succeeded", cached_images: 1 }));
  ...
  it("事务中断和非法图片路径不会留下半目录或不安全缓存文件", async () => {
    ...
    await expect(unsafe.sync.synchronize({ cacheImageScryfallIds: [card.id] })).rejects.toThrow("扩展名");
    expect(unsafe.database.prepare("SELECT COUNT(*) AS count FROM card_printings").get()).toEqual({ count: 0 }); unsafe.database.close();
  });
