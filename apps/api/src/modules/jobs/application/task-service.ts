...
/**
 * I33B：每个 `pack.opened` fact 事件唯一投递一次系列收集率成就处理。任务至少执行一次，
 * `achievement_unlocks` 的 (user_id, definition_id) 唯一约束和 `last_evaluated_fact_id`
 * 去重收敛业务结果，使历史事件补跑不会重复解锁或发奖。
 */
export function enqueuePackAchievementProcessJob(
  database: ConstructorParameters<typeof SqliteJobRepository>[0],
  factEventId: string,
  runAfter: string
): void {
  new SqliteJobRepository(database).enqueue({ type: "pack-achievement.process", payload: { factEventId }, uniqueKey: `pack-achievement.process:${factEventId}`, runAfter, maxAttempts: 5 }, runAfter);
}
