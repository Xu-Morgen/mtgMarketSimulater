...
export type ScryfallBulkSource = {
  version: string;
  downloadUri: string;
...
/** 支持单个 default_cards 描述器，也兼容误配为 /bulk-data 列表入口时的标准列表响应。 */
function selectDefaultCardsMetadata(payload: unknown): ScryfallBulkMetadata {
  if (!isRecord(payload)) throw new Error("Scryfall Bulk 元数据格式无效");
  if (!Array.isArray(payload.data)) return payload;
  const entry = payload.data.find(
    (candidate) => isRecord(candidate) && candidate.type === "default_cards"
  );
  if (!entry) throw new Error("Scryfall Bulk 元数据未找到 default_cards 条目");
  return entry;
}

function retainCard(card: unknown, enabled: ReadonlySet<string> | null): card is ScryfallBulkCard {
  if (!enabled || !isRecord(card) || typeof card.set !== "string") return true;
  return enabled.has(card.set.toUpperCase());
}
...
function parseBulkCards(
  bytes: Buffer,
  enabledSetCodes: readonly string[] | undefined
): ScryfallBulkCard[] {
  const selected: ScryfallBulkCard[] = [];
  const enabled = enabledSetCodes
    ? new Set(enabledSetCodes.map((code) => code.toUpperCase()))
    : null;
  let index = 0;
  while (
    index < bytes.length &&
  ...
  for (index += 1; index < bytes.length; index += 1) {
    ...
    if (depth === 0) {
      const card = JSON.parse(bytes.subarray(objectStart, index + 1).toString("utf8")) as unknown;
      // 字段缺失的对象也必须交给 application 层报出稳定 Schema 错误，不能被筛选静默吞掉。
      if (retainCard(card, enabled)) selected.push(card);
      objectStart = -1;
    }
  }
...
/** Scryfall 新版 jsonl_download_uri 每行是一张卡；只解码当前行，避免创建整份 Bulk 文本。 */
function parseJsonlBulkCards(
  bytes: Buffer,
  enabledSetCodes: readonly string[] | undefined
): ScryfallBulkCard[] {
  const selected: ScryfallBulkCard[] = [];
  ...
  for (let index = 0; index <= bytes.length; index += 1) {
    ...
    let line = bytes.subarray(lineStart, lineEnd);
    if (lineStart === 0 && line[0] === 0xef && line[1] === 0xbb && line[2] === 0xbf)
      line = line.subarray(3);
    if (line.length > 0 && line.some((byte) => byte !== 0x20 && byte !== 0x09 && byte !== 0x0d)) {
      const card = JSON.parse(line.toString("utf8")) as unknown;
      if (retainCard(card, enabled)) selected.push(card);
    }
    lineStart = index + 1;
  }
...
export class ScryfallBulkClient {
  ...
  async download(enabledSetCodes?: readonly string[]): Promise<ScryfallBulkSource> {
    const descriptor = await this.fetcher(this.endpoint, {
      headers: { accept: "application/json", "user-agent": this.userAgent }
    });
    if (!descriptor.ok) throw new Error(`Scryfall Bulk 元数据请求失败：HTTP ${descriptor.status}`);
    const metadata = selectDefaultCardsMetadata(await descriptor.json());
    const downloadUri =
      typeof metadata.jsonl_download_uri === "string"
        ? metadata.jsonl_download_uri
        : metadata.download_uri;
    const isJsonl = typeof metadata.jsonl_download_uri === "string";
    if (typeof metadata.updated_at !== "string" || typeof downloadUri !== "string")
      throw new Error("Scryfall Bulk 元数据缺少版本或下载地址");
    let lastFailure: Error | null = null;
    for (let attempt = 1; attempt <= 3; attempt += 1) {
      try {
        const response = await this.fetcher(downloadUri, {
        ...
        if (
          ...
          throw new Error(`响应长度为 ${bytes.length} 字节，预期 ${expectedLength} 字节`);
        const decoded = bytes[0] === 0x1f && bytes[1] === 0x8b ? gunzipSync(bytes) : bytes;
        const cards = isJsonl
          ? parseJsonlBulkCards(decoded, enabledSetCodes)
          : parseBulkCards(decoded, enabledSetCodes);
        return {
    ...
    throw new Error(
      `Scryfall Bulk 文件损坏或截断，下载或解析失败（已重试 3 次）：${lastFailure?.message ?? "未知错误"}`
    );
  }
}
...
export function scryfallPrintingId(cardId: string): string {
  if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(cardId))
    throw new Error("Scryfall 卡牌 ID 非法");
  return cardId;
}
