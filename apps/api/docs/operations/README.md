...
- 仅在确认可调用受控 Provider 时，运维人员可设置 `LEYLINE_INTEGRATION_TEST=1` 后执行 `pnpm --filter @mtg-market/api test -- --run src/modules/decks/infrastructure/leyline-client.integration.test.ts`。该测试读取 API `.env` 的 `LEYLINE_ENDPOINT`、超时与重试配置，默认应为 `https://api.mtgleyline.com/api/deck-ranking/analyze`；它提交固定的最小 Commander 文本并验证评分、`missingCards` 的 null→空数组归一化和 SHA-256 摘要。默认跳过，防止 CI 或常规测试意外访问外部服务。成功不记录 Provider 原始响应；失败由 `LeylineEvaluationError.reason` 区分 `timeout`、`network`、`http_status`、`invalid_json`、`invalid_schema` 与 `unknown`。
