...
- worker 每次通过 SQLite 条件更新领取一项任务，并为该 attempt 写入运行记录。`running` 任务以租约保护；启动时或领取前会将过期租约恢复到 `pending`，耗尽尝试次数则转为 `dead`。处理器必须把经济结果设计为幂等，worker 提供的是至少执行一次的调度，不替代业务事实唯一约束。
...
- `platform/external/scryfall` 是唯一可访问 Scryfall Bulk Data 与卡图 URL 的适配器。浏览器目录路由只读取 SQLite；任务下载完成后校验 JSON、启用系列、印刷 ID、工艺与可选 checksum，绝不把 Provider 原文转发给客户端。
