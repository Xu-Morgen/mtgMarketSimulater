...
| **AT-11** 基础市场事件系数不突破上限、不改外部快照；风控拦截异常订单 | `packages/rules/src/index.ts` 市场事件/系数上限单测、`order-risk/v1` 单测；`apps/api/src/modules/orders/api/order-routes.test.ts`（越界价格、冷却、频率/数量、自买自卖、高频撤单标记） | I21F 玩家风控提示与管理端只读复核页面待实现（I21B 服务端已拦截） | ✅ 后端通过；I21F 浏览器待实现 |
